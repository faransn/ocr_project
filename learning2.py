...
sgd = SGD()
prob = RMSprop()
adagrad = keras.optimizers.Adadelta()
model.compile(loss='binary_crossentropy', optimizer=prob)
# class_weight = {0: 1, 1: 1000}
class_weight = {0: 1, 1: 1000}  # bokonesh 100
model.fit(X_train, Y_train, batch_size=128, epochs=3, class_weight = class_weight)  # more batch_size require more memory
# 200 250
...
def compare_result(pred, y_test_input):
    for i in range(len(pred)):
        if pred[i] == y_test_input[i]:
            print "It is correct"
        else:
            print "Not correct"
